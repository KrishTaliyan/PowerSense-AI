  ...
  return (
    <section className="panel table-panel">
      ...
      <div className="telemetry-summary">
        <span><strong>{telemetry.filter((row) => !row.is_duplicate && !row.is_stale).length}</strong> clean packets <HelpTooltip label="Clean packets">Messages accepted into pole state after sequence validation.</HelpTooltip></span>
        <span><strong>{telemetry.filter((row) => row.is_duplicate).length}</strong> duplicates quarantined <HelpTooltip label="Duplicate packets">Same device sequence seen again. Stored for audit, ignored for state changes.</HelpTooltip></span>
        <span><strong>{telemetry.filter((row) => row.is_stale).length}</strong> stale packets ignored <HelpTooltip label="Stale packets">Older sequence than the current device state. Stored for audit, ignored for localization.</HelpTooltip></span>
      </div>
      <div className="table-scroll"><table className="data-table"><thead><tr><th>Received</th><th>Source</th><th>Event</th><th>Power state</th><th>Sequence <HelpTooltip label="Sequence">Per-device monotonic counter used to detect duplicate and stale messages.</HelpTooltip></th><th>Quality</th></tr></thead><tbody>
